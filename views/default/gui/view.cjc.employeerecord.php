...
$strModuleName = 'cjc.employeerecord';
    ...
    (function ($) {
        $.widget("custom.combobox", {
            ...
            _createAutocomplete: function () {
                $('ul[class="ui-autocomplete ui-menu ui-widget ui-widget-content ui-corner-all"]').css('max-height', '300px').css('max-width', $($(this)[0].element[0]).css('width')).css('overflow', 'auto').css('box-shadow', '1px 1px 5px rgb(171, 161, 161)');
                var ccbo = this;
...
//                    $('a[id="custom-combobox-showall'+$(this).attr('id').split('custom-combobox-input-').join('')+'"]').click();
                ...
                this.input.bind('autocompletechange', function (event, ui) {
                    ...
                    try {
                        ...
                        if (valid) {
//                            loadList('tblEmployeeInformationList', 'pgEmployeeInformationList');
                            return;
                        }
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        loadList('tblEmployeeInformationList', 'pgEmployeeInformationList', 'No Display');
...
$('#divAdditionalFilter').contents().remove();
        ...
        $("#txtSearchEmployee").keyup(event, function () {
            if (event.keyCode == 13) {
                loadList('tblEmployeeInformationList', 'pgEmployeeInformationList', $("#txtSearchEmployee").val());
            }
        });
...
//                });
//                $("#emp_signature img").attr("src", '../../../documents/EmployeeSignature/' + GlobalIDDataSeledted["emp_id"] + '.png' + "?time=" + new Date());

                //        console.log(data['mname']);
                //Front Populate
                $('div[id="emp_name"] span').text($('#txtfname').val().toString().toUpperCase() + ' ' + $('#txtmname').val().toString().toUpperCase().slice(0, 1) + "." + ' ' + $.trim($('#txtlname').val().toString().toUpperCase()).split(' ').join(' '));
...
//        $('div[id="emp_president"] span').text('PRESIDENT');
                ...
                if (file_exists("../../../documents/EmployeePictures/" + $("#txtemp_id").val() + ".png")) {
                    $('#emp_pic img').attr('src', '../../../documents/EmployeePictures/' + $('#txtemp_id').val() + '.png' + "?time=" + new Date());
                } else {
                    //Picture not exist.
                    $('#emp_pic img').attr("src", '../../../documents/EmployeePictures/nopic.png' + "?time=" + new Date());
                }
                if (file_exists("../../../documents/EmployeeSignature/" + $('#txtemp_id').val() + ".png")) {
                    $('#emp_signature img').attr("src", '../../../documents/EmployeeSignature/' + $('#txtemp_id').val() + ".png" + "?time=" + new Date());
                } else {
                    //Picture not exist.
                    $('#emp_signature img').attr("src", '../../../documents/EmployeeSignature/nopic.png' + "?time=" + new Date());
                }
...
//                });
        ...
        $('button[name="btnSaveEmployee"],button[name="btnPrintID"],button[name="btnCancel"],button[name="btnDetailRefresh"]').bind("click", function (event) {

            if (event.ctrlKey)
                return;
            if ($(this).attr('name') === "btnSaveEmployee" && isAllowed()) {
                saveEmployeeInformationList(event);
            } else if ($(this).attr('name') === "btnCancel") {
                $('div[aria-labelledby$="employeeprofile_dialog"] span[class="ui-icon ui-icon-closethick"]').click()
        ...
        $('button[name="btnRefresh"]').bind("click", function (e) {
            if (e.ctrlKey)
                return;
            if (isAllowedAccessClick('module', 'Employee Record', 'view')) {
                loadList('tblEmployeeInformationList', 'pgEmployeeInformationList', $("#txtSearchEmployee").val());
            }
        });
        $('button[name="btnPreviewID"]').bind("click", function (e) {
            if (e.ctrlKey)
                return;
            if (isAllowedAccessClick('module', 'Employee Record', 'print')) {
                if ($('input[class="cbox"]').length > 0) {
                    EmployeeFramePopupID('', 'frmEmployeeIDPreview', 'Employee ID Preview', 'frmEmployeeIdPreview.php');
                } else {
                    animation(1);
                    var rowdata = jQuery("#tblEmployeeInformationList").jqGrid('getRowData', jQuery("#tblEmployeeInformationList").jqGrid('getGridParam', 'selrow'));
                    if ($.isEmptyObject(rowdata)) {
                        swal("System Message", "No Item selected.", 'warning');
                    } else {
                        EmployeeFramePopupID('', 'frmEmployeeIDPreview', 'Employee ID Preview', 'frmEmployeeIdPreview.php', true, rowdata["emp_id"]);
                    }
                    animation(0);
                }
            }
...
//                console.log(sqlQuery);
        ...
        $('button[name="btnNew"]').bind("click", function (e) {
            if (e.ctrlKey)
                return;
            if (isAllowedAccessClick('module', 'Employee Record', 'create')) {
                popupDialogForm(e, 'employeeprofile_dialog', null);
            }
        });
        $('button[name="btnEdit"]').bind("click", function (e) {
            if (e.ctrlKey)
                return;
            if (isAllowedAccessClick('module', 'Employee Record', 'update')) {
                var rowdata = jQuery('#tblEmployeeInformationList').jqGrid('getRowData', jQuery('#tblEmployeeInformationList').jqGrid('getGridParam', 'selrow'));
                if ($.isEmptyObject(rowdata)) {
                    swal("System Message", "No Item selected.", 'warning');
                } else {
                    popupDialogForm(e, 'employeeprofile_dialog', rowdata);
                }
            }
        });

        //Initial Format css jquery functions
        $('#employeeprofile_dialog input,select').css({"height": "28px", "font-size": "12px", "padding": "5px", "font-weight": "bolder"});
        $('#employeeprofile_dialog select').css({"padding": "0px"});
...
//txtfilterdesignation        
        ...
        $('#employeefilterprintulIDsDropdowns li').unbind();
        $('#employeefilterprintulIDsDropdowns li').bind("click", function (event) {
...
//            console.log($(this).find('a').attr('id'));
    ...
    function PrintPreviewIDIndividual() {
        if (isAllowedAccessClick('module', 'Employee Record', 'print')) {
            animation(1);
            EmployeeFramePopupID('', 'frmEmployeeIDPreview', 'Employee ID Preview', 'frmEmployeeIdPreview.php', true, $('#txtemp_id').val());
            animation(0);
...
//                                reloadtblEmployeeInformationList();
...
//            clickLabel();
    ...
    function functionRefreshEmployeeSignature() {
//
...
//        }, '');
        ...
        var fd = new FormData(document.forms["form1EmployeeSignature"]);
...
//                    console.log(e);
    ...
    function reloadtblEmployeeInformationList() {
        $("div#divEmployeeInformationList table#tblEmployeeInformationList").setGridParam({
            postData: {
                ACTION: 'loadList',
                GETPARAM: {
                    _searchKey: $("#txtSearchEmployee").val(),
    ...
    function loadList(gridID, pgID, searchkey) {
        ...
        $("#" + gridID).jqGrid({
            url: '../../../models/mod.cjc.employeerecord' + ($('button[name="btnFiltersType"]').text() == "FILTER BY" ? '123' : '') + '.php',
            datatype: 'json',
            async: false,
            mtype: 'GET',
            postData: {
            ...
            colModel: [
                {
                    ...
                    hidden: false,
                    align: 'center',
                    label: '<label style="font-size: 12px;" id="<?= $strModuleName; ?>-LBL_TBLEMPLOYEEID"><?= $LBL_TBLEMPLOYEEID; ?></label>',
                    width: 100,
                ...
                },
                {
                    ...
                    formatter: function (cellvalue, options, rowObject) {
                        var imgSrc = '';
                        if (file_exists("../../../documents/EmployeePictures/" + rowObject.emp_id + ".png")) {
                            imgSrc = '../../../documents/EmployeePictures/' + rowObject.emp_id + '.png?' + new Date();
                        } else {
                            imgSrc = '../../../documents/EmployeePictures/nopic.png?' + new Date();
                        }
                ...
                },
                {
                    ...
                    formatter: function (cellvalue, options, rowObject) {
                        var imgSrc = '';
                        if (file_exists("../../../documents/EmployeeSignature/" + rowObject.emp_id + ".png")) {
                            imgSrc = '../../../documents/EmployeeSignature/' + rowObject.emp_id + '.png?' + new Date();

                        } else {
                            imgSrc = '../../../documents/EmployeeSignature/nopic.png?' + new Date();
                        }
                        return '<img id = "imgemployeeImageGrid-' + rowObject.emp_id + '" ' +
                                'src = "' + imgSrc + '"' +
            ...
            height: $(window).height() - parseInt($('#header').css('height').split('px').join('')) - 100,
            pager: "#" + pgID,
            jsonReader: {
                repeatitems: false
            },
            ...
            ondblClickRow: function (id) {
                if (isAllowedAccessClick('module', 'Employee Record', 'update')) {
                    ...
                    popupDialogForm('', 'employeeprofile_dialog', rv);
                }
            },
            loadComplete: function () {
                $(this).contextMenu('EmployeeRecordContextRmenu', '#divEmployeeInformationList', {
                    bindings: {
                        'mnuadd': function (row) {
                            if (isAllowedAccessClick('module', 'Employee Record', 'create')) {
                                popupDialogForm('', 'employeeprofile_dialog', null);
                            }
                        },
                        ...
                        'mnuedit': function (row) {
                            if (isAllowedAccessClick('module', 'Employee Record', 'update')) {
                                ...
                                if ($.isEmptyObject(rowdata)) {
                                    ...
                                    popupDialogForm('', 'employeeprofile_dialog', rowdata);
                                }
                            }
...
//                        height: '130px'
    ...
    function setTableListResizeGrids() {
        $("#tblEmployeeInformationList").setGridWidth($(window).width() - parseInt($('#sidebar').css('width').split('px').join('')) - 8, false);
        $('#tblEmployeeInformationList').jqGrid('setGridHeight', $(window).height() - parseInt($('#header').css('height').split('px').join('')) - parseInt($('#divIDFilterEmployeeContainer').css('height').split('px').join('')) - 100);
    }
    ...
    function EmployeeFramePopupID(event, frmtitle, title, url, isindividual, empid) {
        if (event.ctrlKey)
            return;
        var dwidth = $(window).width() - $(window).width() * 0.29;
        var dheight = $(window).height() - $(window).height() * 0.08;
                    ...
                    open: function () {
                        ...
                        $(this).load(function () {
                            ...
                            if (isindividual == true) {
                                ...
                                var gridRow;
                                var arrData = [];
                                var strRowIDs = $("#tblEmployeeInformationList").jqGrid('getGridParam', 'selarrrow');
                                ...
                                for (var i = 0; i < arrRowIDs.length; i++) {
                                    ...
                                    gridRow = $("#tblEmployeeInformationList").getRowData(strRowID);
                                    var arrValItem = {};
                                    var isFlag = true;
...
//                $('div[role="dialog"][aria-labelledby$="employeeprofile_dialog"]').css('z-index', parseInt($('#header  .navbar').css('z-index')) + 30);
...
//                $(this).dialog('option', 'position', ['middle', 200]);
                ...
                $('div[id="employeeprofile_dialog"] input:not(input[id$="isactive"],#txtSearchEmployee),div[id="employeeprofile_dialog"] select,div[id="employeeprofile_dialog"] textarea').map(function (index, elem) {
...
//txtfilterdesignation        
                ...
                if (data == null) {
//                    $('div[id="employeeprofile_dialog"] input,select,textarea:not(input[id$="isactive"],#txtSearchEmployee)').val('');
//                    $('div[id="employeeprofile_dialog"] select,input:not(input[id="txtemp_id"],input[id$="isactive"],#txtSearchEmployee)').removeAttr('disabled').removeAttr('readonly');
//                    removeAttachement(event);
//                    $('input[name="ID"]').val("");
//                    $('input[name="ID"]').attr('readonly', false).attr('disabled', false);
//
...
//                    LoadPersonInfoImageSettings('tblImageSettings', 'pgImageSettings');
                ...
                //$('div[aria-labelledby$="employeeprofile_dialog"] div[class="panel-body"] div:not(div[id="divPicImageThumb"])').css({"padding": "0px", "padding-left": "5px"});
...
//                    var options = "<option value='" + value['person_code'] + "' >" + pad(value['person_code'], 6) + " - " + value['EmployeeName'] + "</option>";
//                    $('#' + strCboType).append(options);
                        var inputs = $('div[id="employeeprofile_dialog"] input:not(input[id$="isactive"],input[id$="isforprint"],#txtSearchEmployee),div[id="employeeprofile_dialog"] select,div[id="employeeprofile_dialog"] textarea');
...
//                        console.log(type__, name);
                        ...
                        $("#employeeprofile_dialog #imgempPic").removeAttr('src');


                        if (file_exists("../../../documents/EmployeePictures/" + value["emp_id"] + ".png")) {
                            $("#employeeprofile_dialog #imgempPic").attr("src", '../../../documents/EmployeePictures/' + value["emp_id"] + ".png" + "?time=" + new Date());
                        } else {
                            //Picture not exist.
                            if (file_exists("../../../documents/EmployeePictures/" + $("#txtemp_id").val() + ".png")) {
                                $("#employeeprofile_dialog #imgempPic").attr("src", '../../../documents/EmployeePictures/' + value["emp_id"] + ".png" + "?time=" + new Date());
                            } else {
                                $("#employeeprofile_dialog #imgempPic").attr("src", '../../../documents/EmployeePictures/nopic.png' + "?time=" + new Date());
                            }
                        }



                        $('div[id="employeeprofile_dialog"] select,input:not(input[id="txtemp_ida"],input[id$="isactive"],input[id$="isforprint"],#txtSearchEmployee)').removeAttr('disabled').removeAttr('readonly');
        ...
        if (recid == null) {
            $('input[name="ID"]').attr('disabled', false).attr('readonly', false);
...
//        console.log(data['mname']);
        //Front Populate      
        try {
            $('div[id="emp_name"] span').text(data['fname'].toString().toUpperCase() + ' ' + ($.trim(data['mname']) == "." ? "" : data['mname'].toString().toUpperCase().slice(0, 1) + ".") + ' ' + $.trim(data['lname'].toString().toUpperCase()).split(' ').join(' '));
...
//        $('div[id="emp_president"] span').text('PRESIDENT');
        ...
        try {
            if (file_exists("../../../documents/EmployeePictures/" + $("#txtemp_id").val() + ".png")) {
                $('#emp_pic img').attr('src', '../../../documents/EmployeePictures/' + data["emp_id"] + '.png' + "?time=" + new Date());
            } else {
                //Picture not exist.
                $('#emp_pic img').attr("src", '../../../documents/EmployeePictures/nopic.png' + "?time=" + new Date());
            }
            if (file_exists("../../../documents/EmployeeSignature/" + data["emp_id"] + ".png")) {
                $('#emp_signature img').attr("src", '../../../documents/EmployeeSignature/' + data["emp_id"] + ".png" + "?time=" + new Date());
            } else {
                //Picture not exist.
                $('#emp_signature img').attr("src", '../../../documents/EmployeeSignature/nopic.png' + "?time=" + new Date());
            }
    ...
    function saveEmployeeInformationList(event) {
        if (event.ctrlKey) {
            return;
        }
//        debugger;
...
//////#3
        ...
        msgBox(message, "Confirmation", "ask", "Yes|No", 300, 250, function (dlgvalue) {
            ...
            if (dlgvalue == 1) {
                animation(1);
                $.ajax({
                    url: "../../../models/mod.cjc.employeerecord.php?ACTION=saveEmployeeInformation",
                    type: "POST",
                    data: {
                        POSTPARAM: {
                            idnum: $('#txtidnum').val(),
                            ...
                            designation: $('input[id="custom-combobox-input-txtdesignation"]').val(),
                            ...
                            category: $('input[id="custom-combobox-input-txtcategory"]').val(),
                    ...
                    success: function (recid) {
                        ...
                        if (emp_id !== "") {
                            ...
                            $.ajax({
                                url: '../../../models/mod.cjc.employeerecord.php?ACTION=uploadImage',
                                data: data,
                                cache: false,
                                contentType: false,
                                processData: false,
                                ...
                                success: function (data) {
                                    ...
                                    msgBox(message, "Success", "success", "OK", 300, 250, function (dlgvalue) {
                                        ...
                                        //LoadEmployeeInformationListDetails('');
...
//                                        if (file_exists("../../../documents/EmployeePictures/" + $("#txtemp_id").val() + ".png")) {
//                                            $("#employeeprofile_dialog #imgempPic").attr("src", '../../../documents/EmployeePictures/' + $("#txtemp_id").val() + ".png" + "?time=" + new Date());
//                                        } else {
//                                            $("#employeeprofile_dialog #imgempPic").attr("src", '../../../documents/EmployeePictures/nopic.png' + "?time=" + new Date());
//                                        }
                                        if (dlgvalue == 1) {
        ...
        $("#" + Grid).jqGrid({
            ...
            colModel: [
                ...
                },
                {
                    ...
                    formatter: function (cellvalue, options, rowObject) {
                        var imgSrc = '';
                        if (file_exists("../../../documents/EmployeePictures/" + rowObject["Filename"] + "")) {
                            imgSrc = '../../../documents/EmployeePictures/' + rowObject["Filename"] + '?' + new Date();
                        } else {
                            imgSrc = '../../../documents/EmployeePictures/nopic.png?' + new Date();
                        }
        ...
        $("#" + Grid).jqGrid({
            ...
            colModel: [
                ...
                },
                {
                    ...
                    formatter: function (cellvalue, options, rowObject) {
                        var imgSrc = '';
                        if (file_exists("../../../documents/EmployeeSignature/" + rowObject["Filename"])) {
                            imgSrc = '../../../documents/EmployeeSignature/' + rowObject["Filename"] + '?' + new Date();
    ...
    function checkIsActivePicture(gridID, rowid, chkID) {
        ...
        msgBox("Do you want to set this  Picture to Active?", "Confirm", 'ask', 'Yes|No', 0, 0, function (v) {
            if (v == 1) {
                ...
                document.getElementById('img_foldername').value = 'EmployeePictures';
    ...
    function checkIsActiveSignature(gridID, rowid, chkID) {
        ...
        msgBox("Do you want to set this Signature to active?", "Confirm", 'ask', 'Yes|No', 0, 0, function (v) {
            if (v == 1) {
                ...
                document.getElementById('img_filename').value = $('#txtemp_id').val() + '.png';
                document.getElementById('img_filenamenew').value = gridRow["Filename"];
                document.getElementById('img_foldername').value = 'EmployeeSignature';
...
//:::::::::::::::
    ...
    function setImage(strID) {
        ...
        imgMain.src = '../../../models/mod.cjc.employeerecord.php?ACTION=getImageOriginal&GETPARAM=' + strID;
...
//                    $('div[class^="popover"]').eq(0).css({'display': 'none'});
    ...
    function removeAttachement(event) {
        ...
        if (file_exists("../../../documents/EmployeePictures/" + $("#txtemp_id").val() + ".jpg")) {
            msgBox("Are you sure to remove attachment?", "Confirm", 'ask', 'YES|NO', 0, 0, function (v) {
                if (v == 1) {
                    $.ajax({
                        url: '../../../models/mod.cjc.employeerecord.php?ACTION=removeAttachment',
                        data: {
               ...
               <div id="_front" style="display:inline-block;border-right:black solid thin;   padding: 10px 27px 30px 20px;">\n\
                  <div id="div_img_id_front" style="border-radius:10px;border: rgba(153, 153, 153, 0) dashed 0.5px;width:' + data['idwidth'] + ';height:' + data['idheight'] + ';position:relative;float:left;">\n\
                      ...
                      <div id="emp_pic" style="width:' + data['picwidth'] + ';height:' + data['picheight'] + ';top:0px;position:absolute;margin-top: 81px;right: 8px;text-align: center;">\n\
                          <img src="../../../documents/EmployeePictures/1.png" style="width:' + data['picwidth'] + ';height:' + data['picheight'] + ';border: #C15858 solid thin;border-color: transparent;border-radius: 5px;" alt="">\n\
                      </div> \n\
                 ...
                 <div id="div_img_id_back" style="border-radius:10px;border: rgba(153, 153, 153, 0) dashed 0.5px;width:' + data['idwidth'] + ';height:' + data['idheight'] + ';position:relative;float:right;"> \n\
                    ...
                    <div id="s_birthdate" style="position: absolute;top: 0;margin-top: 85px;left: 45px;font-size: 12px;width: 90%;text-align: left;-webkit-transform: scale(0.7081,0.9175); font-weight: bold;">  <span></span></div>\n\
                     ...
                     <div id="emp_signature" style="width: 100%;position: relative;top: 0;margin-top: -60px;-webkit-transform: scale(0.91,0.975); font-weight: bold;">\n\
                         <img src="../../../documents/EmployeeSignature/1.png?Thu Sep 03 2015 21:51:26 GMT-0700 (Pacific Daylight Time)" style="/*width: 160px;height: 35px;*/height:50px;border: #C15858 solid thin;border-color: transparent;border-radius: 5px;" alt=""> \n\
                     </div> \n\
                </div>\n\
             </div> \n\
          </div> \n\
                ...
                .dialog({
                    ...
                    open: function () {
                        ...
                        $(this).load(function () {
                            ...
                            frame.foldername = 'EmployeePictures';

                            frame.loadInitialCanvas($('#txtemp_id').val(), 'EmployeePictures');
//                            frame.$('button[name="btnPrintID"]').click();
//                            frame.$('button[name="btnPrintID"]').click();
                            if ($('img[id="imgempPic"]').attr('src').split('nopic.png').length == 1) {
                                msgBox("Do you want to replace Picture?", "Confirm", 'ask', 'Yes|No', 0, 0, function (v) {
                ...
                .dialog({
                    ...
                    open: function () {
                        ...
                        $(this).load(function () {
                            ...
                            frame.filename = $('#txtemp_id').val();
                            frame.foldername = 'EmployeeSignature';

                            frame.loadInitialCanvas($('#txtemp_id').val(), 'EmployeeSignature');
...
<div  class="container-fluid">
    <div id="divIDFilterEmployeeContainer" class="panel panel-default" style="margin: 0px;">
                        ...
                        <div class="row-fluid">                          
                            <div class="col-xs-8 text-left" style=" padding: 0px;width:75%;">                                
                                <div id="divFilterType" class="col-xs-4" style=" padding: 0px; width:30%;   padding: 0px 10px 0px 0px;">
                                    <div class="input-group">
                                        ...
                                        <select  id="txtfiltercategory" class="form-control" required="required" style="text-align: center;">                    
                                            ...
                                            <option>ELRIC</option>
                                            <option>LEDOUX</option>
                                            <option>BROTHER POLYCARP</option>
                                        </select>
                                    </div>
                                </div> 
                                ...
                                <div id="divFilterPrint" class="col-xs-1" style="padding: 0px;width: 15%;">
                                    ...
                                    <div id="employeefilterprintbtnFilters" class="btn-group" style="padding: 0px;padding-left: 15%;">
                                        ...
                                        <ul id="employeefilterprintulIDsDropdowns" class="dropdown-menu" role="menu" style="min-width: 120px;margin-left: 13%;">
                                ...
                                <div class="col-xs-4" style=" padding:0px 0px 0px 2.7%;width: 30%;">
                                    <div class="input-group">
                                        <input id="txtSearchEmployee" class="form-control txt-standard" type="text" placeholder="Search" style="height: 28px;">
                                        <span class="input-group-addon" style="padding: 0px;">
                                            <button name="btnSearchEmployee" id="<?= $strModuleName ?>-LBL_REFRESH" title="click to refresh" class="btn btn-default btn-md btn-standard" style="padding: 1px 7px;height: 25px;">
    ...
    <div class="panel-body" id="panel-body" style="padding:0px;">
        <div id="divEmployeeInformationList" oncontextmenu="return false;">
            <table id="tblEmployeeInformationList"></table>
            <div id="pgEmployeeInformationList"></div>
        </div>
    </div>
</div>
<div class="panel panel-default" id="employeeprofile_dialog" style="display: none;padding: 0px;">
    ...
    <div class="panel-heading" id="divTopHeader" style="height: 40px;width: 100%;padding: 0px;">
        <div class="col-md-6" style="float:left;margin-top:3px;"> 
            ...
            <button  id="<?= $strModuleName ?>-LBL_SAVE"  name="btnSaveEmployee" class="btn btn-info btn-lg" title="Save" style="padding: 7px 12px;float: left;font-size: 12px;margin-right: 0.5%;       width: 15%;">
                    ...
                    <div class="col-xs-10" style="padding: 0;">
                        <div class="row-fluid">
                            <div class="col-xs-3" style="padding-right: 0px;">
                                <div class="input-group">
                                    <span class="input-group-addon" style="text-align: left;">
                                        <label  id="<?= $strModuleName; ?>-LBL_FRMEMPLOYEEID" style="    min-width: 84px;"><?= $LBL_FRMEMPLOYEEID; ?></label>
                                    </span>
                                    <input  id="txtidnum" type="hidden"/>
                                    <input name="ID" class="form-control" maxlength="11" type="text" id="txtemp_id" readonly style="text-align: center;"/>
                                </div>
                            </div> 
                            <div class="row-fluid col-xs-9" style="padding: 0px; ">
                                <div class="col-xs-12"> 
                                    <div class="input-group col-xs-2" style="border-bottom-left-radius: 4px;border-top-left-radius: 4px;border-left: 1px solid #ccc;height: 28px;border-bottom-right-radius: 0px;  border-top-right-radius: 0px;  padding: 0px 0px 0px 0px;">
                                        <span class="input-group-addon " style="padding:0px;padding-left: 10px;"><label id="<?= $strModuleName ?>-LBL_FRMEMPLOYEENAME" style="min-width: 123px;float: left;text-align: left;"><?= $LBL_FRMEMPLOYEENAME ?></label></span>
                                    </div>                       
        ...
        <div class="container-fluid text-left" id="divTabs">
            ...
            <div class="tab-content" id='divTabContent' style="width:100%;height:290px;overflow: overlay;padding-right: 17px;" >
                        ...
                        <div role="form" class="form-inline" onsubmit="return false;">                        
                            <div class="row">
                                ...
                                <div class="col-xs-6">
                                    ...
                                    <div class="row" style="margin-top: 0.5%;">
                                        ...
                                        <div class="form-group col-xs-3" style="padding-right:0px;  ">
                                            <label id="<?= $strModuleName ?>-LBL_FRMCATEGORY" style="min-width: 100px;float: left;text-align: left;font-size: 13px;margin-top: 6px;">Company <?= $LBL_FRMCATEGORY ?></label>
                                        </div>
                                        <div class="form-group col-xs-5" style="padding:0;width: 37.5%;">
                                            <select id="txtcompany" class="form-control" required="required" style="text-align: center;">                   
                                                <option>CJC</option>
                                                <option>ELRIC</option>
                                                <option>LEDOUX</option>
                                                <option>BROTHER POLYCARP</option>
                            ...
                            <div class="row">
                                <div class="col-xs-6">
                                    ...
                                    <!--<legend>-->
                                        <!--<label  id="<?= $strModuleName ?>-LBL_PRSONINFOADDRESS" style="float: left;text-align: left;color: maroon;font-weight: bolder;font-size: 12px;    margin-top: -3px;"><?= $LBL_PRSONINFOADDRESS ?></label>-->
                ...
                <div class="panel panel-default tab-pane fade " id="tab-IDPREVIEW" style="border: none;">                 
                    ...
                    <button id="btnRefreshSignature" style="display:none;" onclick="functionRefreshEmployeeSignature();">Refresh Signature</button>
                                ...
                                <div id="_front" style="display:inline-block;border-right:black solid thin;   padding: 10px 20px 30px 20px;">
                                    <div id="div_img_id_front" style="border-radius:10px;border: rgba(153, 153, 153, 0) dashed 0.5px;width: 5.4cm;height: 8.54cm;position:relative;float:left;">
                                        ...
                                        <div id="emp_pic" style="width: 100px;height: 100px;top:0px;position:absolute;margin-top: 81px;right: 8px;text-align: center;">
                                            <img src="../../../documents/EmployeePictures/nopic.png?time=Fri Mar 04 2016 00:00:42 GMT-0800 (Pacific Standard Time)" style="width: 97px;height: 97px;border: #C15858 solid thin;border-color: transparent;border-radius: 5px;" alt="">
                                        </div> 
                                ...
                                <div id="_back" style="display: inline-block;    padding: 10px 0px 30px 20px;"> 
                                    <div id="div_img_id_back" style="border-radius:10px;border: rgba(153, 153, 153, 0) dashed 0.5px;width: 5.4cm;height: 8.54cm;position:relative;float:right;"> 
                                        ...
                                        <div id="emp_signature" style="width: 100%;position: relative;top: -60px;-webkit-transform: scale(0.91,0.975); font-weight: bold;">
                                            <img src="../../../documents/EmployeeSignature/0017-0222-5.png?time=Fri Mar 04 2016 00:00:42 GMT-0800 (Pacific Standard Time)" style="width: 160px;height: 35px;border: #C15858 solid thin;border-color: transparent;border-radius: 5px;" alt=""> 
                                        </div> 
                                    </div>
                                </div> 
                            </div>
                                                            ...
                                                            <div id="_front" style="display:inline-block;border-right:black solid thin;   padding: 10px 20px 30px 20px;">
                                                                <div id="div_img_id_front" style="border-radius:10px;border: rgba(153, 153, 153, 0) dashed 0.5px;width: 5.43cm;height: 8.58cm;position:relative;float:left;">
                                                                    ...
                                                                    <div id="emp_pic" style="width: 100px;height: 100px;top:0px;position:absolute;margin-top: 81px;right: 10px;text-align: center;">
                                                                        <img src="../../../documents/EmployeePictures/nopic.png?time=Wed Sep 16 2015 20:33:52 GMT-0700 (Pacific Daylight Time)" style="width: 100px;height: 100px;border: #C15858 solid thin;border-color: transparent;border-radius: 5px;" alt="">
                                                                    </div> 
                                                            ...
                                                            <div id="_back" style="display: inline-block;  padding: 10px 0px 30px 20px;"> 
                                                                <div id="div_img_id_back" style="border-radius:10px;border: rgba(153, 153, 153, 0) dashed 0.5px;width: 5.43cm;height: 8.58cm;position:relative;float:right;"> 
                                                                    ...
                                                                    <div id="emp_signature" style="width: 100%;position: relative;top: -47px;-webkit-transform: scale(0.91,0.975); font-weight: bold;">
                                                                        <img src="../../../documents/EmployeeSignature/2009-0418-8.png?Thu Sep 03 2015 21:51:26 GMT-0700 (Pacific Daylight Time)" style="width: 128px;height: 20px;border: #C15858 solid thin;border-color: transparent;border-radius: 5px;" alt=""> 
                                                                    </div> 
                                                                </div>
                                                            </div> 
                                                        </div> -->
...
<!--                        <div id="theBox"><span>This is the content</span></div>
    ...
    <form method="post" accept-charset="utf-8" name="form1EmployeeSignature" style="display:none;">    
...
<div class="contextMenu" class="vmenu"  id="EmployeeRecordContextRmenu"  style="display:none;">
