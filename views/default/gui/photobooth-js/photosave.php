...
if (file_exists($fullpath)) {
    ...
    if ($success) {
        if ($_POST['img_foldername'] == "AlumniPictures") {
            $dbname = DB_NAME;
            $strSQL = "INSERT INTO `$dbname`.zpicturerecord SET  
              id=null,
        ...
        } else if ($_POST['img_foldername'] == "EmployeePictures") {
            $dbname = DB_NAME;
           $strSQL = "INSERT INTO `$dbname`.zpicturerecord SET  
              id=null,
              emp_id='{$_POST['img_filename']}',
            ...
            ";
        mysql_query($strSQL, $conn) or die(mysql_error() . $strSQL);
        } else if ($_POST['img_foldername'] == "StudentPictures") {
            $dbname = DB_NAME;
             $strSQL = "INSERT INTO `$dbname`.zpicturerecord SET  
              id=null,
              emp_id='{$_POST['img_filename']}',
...
$dfd = file_put_contents($imagePath, $unencodedData);
if ($isExist) {
}else{
    if ($_POST['img_foldername'] == "AlumniPictures") {
        $dbname = DB_NAME;
