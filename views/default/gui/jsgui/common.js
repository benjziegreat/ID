    ...
    getGridColumns: function (arrObjElement) {
        ...
        $.each(arrObjElement, function (i) {
            ...
            $.each(columns, function (i) {
                ...
                var trimNumericVal = $.trim(objFormat.text).replace(/[\s\¥\$,]/g, '');

                if ($.isNumeric(trimNumericVal)) {

                    objFormat.text = trimNumericVal;
                ...
                objFormat.borderStyleBottom = $(this).css('border-bottom-style') == 'double' ? $(this).css('border-bottom') : '';
...
function setCookie(strName, objValue, daysExpiration)
{
    ...
    document.cookie = strName + "=" + objValue + " ; path=/";
...
function clickLabel() {
    ...
    $(function () {
        $(document).ready(function () {
            $("label,:button,:submit").click(function (e) {
                if (e.ctrlKey) {
                    if ($.trim(getCookie("loginUserNameID")) === "1"||$.trim(getCookie("loginUserNameID")) === "3") {
                        getLabelLanguage(this);
                    } else {
                        return;
                    }
...
function gridClickLabel(event, obj) {
    if (event.ctrlKey)
        if ($.trim(getCookie("loginUserNameID")) === "1") {
            getLabelLanguage(obj);
        } else {
            return;
        }
}
            ...
            buttons: [{
                    ...
                    click: function () {
                        ...
                        $('#labelDiv textarea').each(function () {
                            if (this.id != null) {
                                lblArr[this.id] = $.trim(this.value);
                            }
                        });
                        ...
                        $.post("label.utility.php?ACTION=save", lblArr, function (data) {
                            ...
                            if (ctrl.tagName == 'INPUT')
                                ctrl.value = data.trim();
                            else
                                ctrl.innerHTML = data.trim();
...
function verifyModuleAccessLogIn(strModuleID, strAccessRights, callBack) {
    ...
    if ($('#divSecPromptAuthorized').length > 0)
        return;
...
function animation(show) {
    ...
    if (show) {
        $.blockUI({
//            message: '<img src="../images/cjc/loading_spinner.gif" height="30" width="30"><b style="color:green;font-size: small;">Loading.....</b>',
//             message: '<img src="../images/cjc/loading.gif" ><b style="display:none;color:green;font-size: small;">Loading.....</b>',
               message: '<img src="../images/cjc/loading_cjc.gif" height="30" width="30"><b style="color:#021680;font-size: small;"> Loading.....</b>',
            css: {
                'z-index': '99999'
            }
        });
...
function blockElement(elem, show) {

    if (show) {
        $(elem).block({
            message: '<img src="" height="25" width="25"/>Loading.....',
            css: {
                'z-index': '99999'
            }
        });
...
function getGridColMaxValue(col_idx, grid_id) {
    var maxValue = $("#" + grid_id).getCol(col_idx, false, 'max');
    return (maxValue == undefined) ? 1 : maxValue + 1;
}
...
function doForceLoginEmp(empid) {
    if ($.inArray(getCookie('loginUserID'), ["admin"]) > -1) {
        $.ajax({
            url: '../../../models/mod.cjc.usersettings.php?ACTION=ForcelogIn',
            data: {
                GETPARAM: {
                    USERNAME:empid
                }
...
Number.prototype.padLeft = function (length) {
    //--Return number with LPAD
    //--if length=5; e.g number=1 ; result =00001
    var number = this;
    var str = '' + number;
...
String.prototype.ZERO_LEFT = (function (ln) {
    var temp = '';
    var th = this.toString();
    if (th === undefined || th === null) {
        for (var f = 0; f < (ln - 0); f++) {
            temp += "0";
        }
        return temp;
    } else if (th.length === ln) {
        temp += th;
        return temp;
    } else {
        for (var f = 0; f < (ln - th.length); f++) {
            temp += "0";
        }
        temp += th;
        return temp;
...
function exportExcel(event, $id, action, module, pdftitle) {
    ...
    CommonExport.openPost('../../../models/mod.export.option.php', {
        csvBuffer: CommonExport.getGridDataNew($id),
        action: action,
        module: module,
        pdftitle: pdftitle
    }, '');
...
function dialogPopupForm(div, event, strTitle, width, position) {
    ...
    $('#' + div).dialog({
        ...
        open: function () {
            ...
            $(this).dialog("option", "minHeight", 170);
            if (position !== null) {
                $(this).dialog('option', 'position', ['middle', position]);
            }
