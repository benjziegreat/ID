var Leave = (function(_objVariables_,_objElements_,isGrid,tblOptions_){
    ...
    $("div#_div-dialog-browser_").dialog({
        ...
        close: function(){
            if(isGrid){
                if(_objVariables_["leave_code"] != null){
                    var ltyp=false;
                    $.each($("#"+tblOptions_["tbl"]).getRowData(),function(k,v){
                        if($.trim(k) != $.trim(tblOptions_["row"])){
                            if($.trim(v["LeaveTypeID"])== $.trim(_objVariables_["leave_code"])){
                                ltyp = true;
                            }
                        }
                    });
                    if(!ltyp){
                        ...
                        msgBox("Leave Type is Already Selected!","System Message","stop","ok",270,240,function(){
