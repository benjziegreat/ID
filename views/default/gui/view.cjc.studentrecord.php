...
$strModuleName = 'cjc.studentrecord';
...
//                    $('a[id="custom-combobox-showall'+$(this).attr('id').split('custom-combobox-input-').join('')+'"]').click();
                ...
                this.input.bind('autocompletechange', function (event, ui) {
                    ...
                    try {
                        ...
                        if (valid) {
//                            loadList('tblStudentInformationList', 'pgStudentInformationList',$("#txtSearchStudent").val());
                            return;
                        }
...
function LoadValuesOptionFromDB(strCboType,action,fieldforvalue,fieldfortext) {
        ...
        // }));
        $.ajax({
            url: "../../../models/mod.cjc.studentrecord.php",
            data: {
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        iniControls();
        loadList('tblStudentInformationList', 'pgStudentInformationList', "No Display");
        onresize = function () {
...
//        });
        ...
        $("#txtSearchStudent").keyup(event, function () {
            if (event.keyCode == 13) {
                loadList('tblStudentInformationList', 'pgStudentInformationList', $("#txtSearchStudent").val());
            }
        });
        $("button[name='btnSearchStudent']").click(null, function (event) {
            loadList('tblStudentInformationList', 'pgStudentInformationList', $("#txtSearchStudent").val());
        });
...
//                fitTextInBox('emp_position');
           var fullname=$('#txtfname').val().toString().toUpperCase() + ' ' + $('#txtmname').val().toString().toUpperCase().slice(0, 1) + "." + ' ' + $('#txtlname').val().toString().toUpperCase().split(' ').join(' ');
...
//                });
//                $("#emp_signature img").attr("src", '../../../documents/StudentSignature/' + GlobalIDDataSeledted["emp_id"] + '.png' + "?time=" + new Date());
...
//        $('div[id="emp_president"] span').text('PRESIDENT');
                ...
                if (file_exists("../../../documents/StudentPictures/" + $("#txtemp_id").val() + ".png")) {
                    $('#emp_pic img').attr('src', '../../../documents/StudentPictures/' + $('#txtemp_id').val() + '.png' + "?time=" + new Date());
                } else {
                    //Picture not exist.
                    $('#emp_pic img').attr("src", '../../../documents/StudentPictures/nopic.png' + "?time=" + new Date());
                }
                if (file_exists("../../../documents/StudentSignature/" + $('#txtemp_id').val() + ".png")) {
                    $('#emp_signature img').attr("src", '../../../documents/StudentSignature/' + $('#txtemp_id').val() + ".png" + "?time=" + new Date());
                } else {
                    //Picture not exist.
                    $('#emp_signature img').attr("src", '../../../documents/StudentSignature/nopic.png' + "?time=" + new Date());
                }
                ...
                $('div[id="s_mother"] span').text($.trim($('#txtcontact_guardian').val()).toUpperCase());
                $('div[id="p_address"] span').text($('#txtcontact_address').val());
                $('div[id="p_cellno"] span').text($('#txtcontactno').val());
            $('div[id^="emp_name"]').map(function(index, elem) {     
            if($.trim($(elem).find('span').text()).length>23){
                        fitTextInBox(elem);
                      }              
                   });
               ...
               $('div[id^="p_cellno"]').map(function(index, elem) {
                 //console.log($(elem).find('span').text());
                  if($.trim($(elem).find('span').text()).length>22){
                        fitTextInBox(elem);                      }                
...
//                });
        ...
        $('button[name="btnSaveStudent"],button[name="btnPrintID"],button[name="btnCancel"],button[name="btnDetailRefresh"]').bind("click", function (event) {

            if (event.ctrlKey)
                return;
            if ($(this).attr('name') === "btnSaveStudent" && isAllowed()) {
                saveStudentInformationList(event);
            } else if ($(this).attr('name') === "btnCancel") {
                $('div[aria-labelledby$="studentprofile_dialog"] span[class="ui-icon ui-icon-closethick"]').click()
        ...
        $('button[name="btnRefresh"]').bind("click", function (e) {
            if (e.ctrlKey)
                return;
            if (isAllowedAccessClick('module', 'Student Record', 'view')) {
                loadList('tblStudentInformationList', 'pgStudentInformationList', $("#txtSearchStudent").val());
            }
        });
        $('button[name="btnPreviewID"]').bind("click", function (e) {
            if (e.ctrlKey)
                return;
            if (isAllowedAccessClick('module', 'Student Record', 'print')) {
                if ($('input[class="cbox"]').length > 0) {
                    StudentFramePopupID('', 'frmStudentIDPreview', 'Student ID Preview', 'frmStudentIdPreview.php');
                } else {
                    animation(1);
                    var rowdata = jQuery("#tblStudentInformationList").jqGrid('getRowData', jQuery("#tblStudentInformationList").jqGrid('getGridParam', 'selrow'));
                    if ($.isEmptyObject(rowdata)) {
                        swal("System Message", "No Item selected.", 'warning');
                    } else {
                        StudentFramePopupID('', 'frmStudentIDPreview', 'Student ID Preview', 'frmStudentIdPreview.php', true, rowdata["idnum"]); //05132025:  change emp_id to idnum mar 13 2025
                    }
                    animation(0);
                }
            }
...
//                console.log(sqlQuery);
                        openPost('../../../models/ExportExcelTemplate.php', {
                            ...
                            Type: 'hrmsMSTStudentInformation'
                        }, '');
                    } else {
                        return;
                    }
        ...
        $('button[name="btnNew"]').bind("click", function (e) {
            if (e.ctrlKey)
                return;
            if (isAllowedAccessClick('module', 'Student Record', 'create')) {
                popupDialogForm(e, 'studentprofile_dialog', null);
            }
        });
        $('button[name="btnEdit"]').bind("click", function (e) {
            if (e.ctrlKey)
                return;
            if (isAllowedAccessClick('module', 'Student Record', 'update')) {
                var rowdata = jQuery('#tblStudentInformationList').jqGrid('getRowData', jQuery('#tblStudentInformationList').jqGrid('getGridParam', 'selrow'));
                if ($.isEmptyObject(rowdata)) {
                    swal("System Message", "No Item selected.", 'warning');
                } else {
                    popupDialogForm(e, 'studentprofile_dialog', rowdata);
                }
            }
        });

        //Initial Format css jquery functions
        $('#studentprofile_dialog input,select').css({"height": "28px", "font-size": "12px", "padding": "5px", "font-weight": "bolder"});
        $('#studentprofile_dialog select').css({"padding": "0px"});
...
//txtfilterdesignation        
        ...
        $('#studentfilterprintulIDsDropdowns li').unbind();
        $('#studentfilterprintulIDsDropdowns li').bind("click", function (event) {
...
//            console.log($(this).find('a').attr('id'));
            ...
            //console.log($.trim($(this).find('a').attr('id').split('idLink').join('')));
    ...
    function PrintPreviewIDIndividual() {
        if (isAllowedAccessClick('module', 'Student Record', 'print')) {
            animation(1);
            StudentFramePopupID('', 'frmStudentIDPreview', 'Student ID Preview', 'frmStudentIdPreview.php', true, $('#txtidnum').val()); //05132025:  change txtemp_id to txtidnum may 13 2025
            animation(0);
...
//                    animation(1);
//                    $.ajax({
//                        url: "../../../models/mod.cjc.studentrecord.php?ACTION=updateprintinghistory",
//                        type: "POST",
//                        data: {
//                            POSTPARAM: {
//                                idnum: $('#txtidnum').val(),
...
//                                reloadtblStudentInformationList();
...
//            clickLabel();
    ...
    function functionRefreshStudentSignature() {
//
...
//        }, '');
        ...
        var fd = new FormData(document.forms["form1StudentSignature"]);
...
//                    console.log(e);
    ...
    function reloadtblStudentInformationList() {
        $("div#divStudentInformationList table#tblStudentInformationList").setGridParam({
            postData: {
                ACTION: 'loadList',
                GETPARAM: {
                    _searchKey: $("#txtSearchStudent").val(),
    ...
    function loadList(gridID, pgID, searchkey) {
        ...
        $("#" + gridID).jqGrid({
            url: '../../../models/mod.cjc.studentrecord' + ($('button[name="btnFiltersType"]').text() == "FILTER BY" ? '123' : '') + '.php',
            datatype: 'json',
            async: false,
            mtype: 'GET',
            postData: {
            ...
            colModel: [
                {
                    ...
                    sortable:true,
                    align: 'center',
                    label: '<label style="font-size: 12px;" id="<?= $strModuleName; ?>-LBL_TBLSTUDENTID"><?= $LBL_TBLSTUDENTID; ?></label>',
                    width: 100,
                ...
                },
                {
                    ...
                    formatter: function (cellvalue, options, rowObject) {
                        var imgSrc = '';
                        if (file_exists("../../../documents/StudentPictures/" + rowObject.emp_id + ".png")) {
                            imgSrc = '../../../documents/StudentPictures/' + rowObject.emp_id + '.png?' + new Date();
                        } else {
                            imgSrc = '../../../documents/StudentPictures/nopic.png?' + new Date();
                        }
                ...
                },
                {
                    ...
                    formatter: function (cellvalue, options, rowObject) {
                        var imgSrc = '';
                        if (file_exists("../../../documents/StudentSignature/" + rowObject.emp_id + ".png")) {
                            imgSrc = '../../../documents/StudentSignature/' + rowObject.emp_id + '.png?' + new Date();

                        } else {
                            imgSrc = '../../../documents/StudentSignature/nopic.png?' + new Date();
                        }
                        return '<img id = "imgemployeeImageGrid-' + rowObject.emp_id + '" ' +
                                'src = "' + imgSrc + '"' +
                                'style =" vertical-align: middle;max-width: 65px;max-height: 25px;height:28px;" ' +
                                '</img>';
                    },
                    align: "center",
                ...
                },
                {
                    ...
                    formatter: function (cellvalue, options, rowObject) {
//                        return toUpperFirstWord(cellvalue);
                        return  cellvalue == null ? '' : toUpperFirstWord(cellvalue);
                    },
                    unformat: function (cellvalue, options, cell) {
                        return  cellvalue;
                    }
            ...
            height: $(window).height() - parseInt($('#header').css('height').split('px').join('')) - parseInt($('#divIDFilterStudentContainer').css('height').split('px').join('')) - 100,
            pager: "#" + pgID,
            jsonReader: {
                repeatitems: false
            },
            ...
            ondblClickRow: function (id) {
                if (isAllowedAccessClick('module', 'Student Record', 'update')) {
                    ...
                    popupDialogForm('', 'studentprofile_dialog', rv);
                }
            },
            loadComplete: function () {

                $(this).contextMenu('StudentRecordContextRmenu', '#divStudentInformationList', {
                    bindings: {
//                        'mnuadd': function (row) {
//                            if (isAllowedAccessClick('module', 'Student Record', 'create')) {
//                                popupDialogForm('', 'studentprofile_dialog', null);
//                            }
//                        },
                            ...
                            if (isAllowedAccessClick('module', 'Student Record', 'print')) {
                                if ($('input[class="cbox"]').length > 0) {
                                    ...
                                    if ($.isEmptyObject(rowdata)) {
                                        ...
                                        StudentFramePopupID('', 'frmStudentIDPreview', 'Student ID Preview', 'frmStudentIdPreview.php', true, rowdata["idnum"]); //05132025:  change emp_id to idnum may 13 2025
                                    }
                                    animation(0);
                                }
                            }
                        ...
                        'mnuedit': function (row) {
                            if (isAllowedAccessClick('module', 'Student Record', 'update')) {
                                ...
                                if ($.isEmptyObject(rowdata)) {
                                    ...
                                    popupDialogForm('', 'studentprofile_dialog', rowdata);
                                }
                            }
...
//                        height: '130px'
    ...
    function setTableListResizeGrids() {
        $("#tblStudentInformationList").setGridWidth($(window).width() - parseInt($('#sidebar').css('width').split('px').join('')) - 8, false);
        $('#tblStudentInformationList').jqGrid('setGridHeight', $(window).height() - parseInt($('#header').css('height').split('px').join('')) - parseInt($('#divIDFilterStudentContainer').css('height').split('px').join('')) - 100);
    }
    ...
    function StudentFramePopupID(event, frmtitle, title, url, isindividual, empid) {
        if (event.ctrlKey)
            return;
        var dwidth = $(window).width() - $(window).width() * 0.29;//0.25
        var dheight = $(window).height() - $(window).height() * 0.08;//0.10
                    ...
                    open: function () {
                        ...
                        $(this).load(function () {
                            ...
                            if (isindividual == true) {
                                frame.employeeData = empid;//arrData;
                                frame.typeData='idnum'; //05132025:  updated may 13 2025
                                frame.initialLoad();
                                //  frame.$('button[name="btnPrintID"]').click();
                            } else {
                                var gridRow;
                                var arrData = [];
                                var strRowIDs = $("#tblStudentInformationList").jqGrid('getGridParam', 'selarrrow');
                                ...
                                for (var i = 0; i < arrRowIDs.length; i++) {
                                    ...
                                    gridRow = $("#tblStudentInformationList").getRowData(strRowID);
                                    var arrValItem = {};
                                    var isFlag = true;
                                    ...
                                    if (isFlag) {
                                        ...
                                        emp_ids = emp_ids + ",'" + gridRow["idnum"] + "'"; // 05132025: change gridRow["emp_id"] to gridRow["idnum"] updated may 13 2025
                                    }
...
//                            frame.$('button[name="btnPrintID"]').click();
    ...
    function popupDialogForm(event, div, data) {
        ...
        $('#' + div).dialog({
            ...
            modal: true,
            open: function (event) {
            console.log('Open popup student profile');
//                $('div[role="dialog"][aria-labelledby$="studentprofile_dialog"]').css('z-index', parseInt($('#header  .navbar').css('z-index')) + 30);
...
//                $(this).dialog('option', 'position', ['middle', 200]);
                ...
                $('div[id="studentprofile_dialog"] input,select,textarea:not(input[id$="isactive"],#txtSearchStudent)').map(function (index, elem) {
                ...
                if (data == null) {
//                    $('div[id="studentprofile_dialog"] input,select,textarea:not(input[id$="isactive"],#txtSearchStudent)').val('');
//                    $('div[id="studentprofile_dialog"] select,input:not(input[id="txtemp_id"],input[id$="isactive"],#txtSearchStudent)').removeAttr('disabled').removeAttr('readonly');
...
//                    LoadPersonInfoImageSettings('tblImageSettings', 'pgImageSettings');
                    ...
                    GetDetailList(data["idnum"],'idnum'); //05132025: change emp_id to idnum mar 13 2025
                    $('#btnPrintPreviewID').attr('disabled', false);
                }
                $('#txtworkstatus').attr('readonly', true).attr('disabled', true);
                //$('div[aria-labelledby$="studentprofile_dialog"] div[class="panel-body"] div:not(div[id="divPicImageThumb"])').css({"padding": "0px", "padding-left": "5px"});
...
//                    $('#' + strCboType).append(options);
                        var inputs = $('div[id="studentprofile_dialog"] input:not(input[id$="isactive"],input[id$="isforprint"],#txtSearchStudent),div[id="studentprofile_dialog"] select,div[id="studentprofile_dialog"] textarea');
...
//                        console.log(type__, name);
                        ...
                        $("#studentprofile_dialog #imgempPic").removeAttr('src');


                        if (file_exists("../../../documents/StudentPictures/" + value["emp_id"] + ".png")) {
                            $("#studentprofile_dialog #imgempPic").attr("src", '../../../documents/StudentPictures/' + value["emp_id"] + ".png" + "?time=" + new Date());
                        } else {
                            //Picture not exist.
                            if (file_exists("../../../documents/StudentPictures/" + $("#txtemp_id").val() + ".png")) {
                                $("#studentprofile_dialog #imgempPic").attr("src", '../../../documents/StudentPictures/' + value["emp_id"] + ".png" + "?time=" + new Date());
                            } else {
                                $("#studentprofile_dialog #imgempPic").attr("src", '../../../documents/StudentPictures/nopic.png' + "?time=" + new Date());
                            }
                        }



                        $('div[id="studentprofile_dialog"] select,input:not(input[id="txtemp_id"],input[id$="isactive"],input[id$="isforprint"],#txtSearchStudent)').removeAttr('disabled').removeAttr('readonly');
...
//        console.log(data['mname']);
        //Front Populate
        var fullname=data['fname'].toString().toUpperCase() + ' ' + ($.trim(data['mname']) == "." ? "" : data['mname'].toString().toUpperCase().slice(0, 1) + ".") + ' ' + data['lname'].toString().toUpperCase();
...
//        $('div[id="emp_president"] span').text('PRESIDENT');
        ...
        if (file_exists("../../../documents/StudentPictures/" + $("#txtemp_id").val() + ".png")) {
            $('#emp_pic img').attr('src', '../../../documents/StudentPictures/' + data["emp_id"] + '.png' + "?time=" + new Date());
        } else {
            //Picture not exist.
            $('#emp_pic img').attr("src", '../../../documents/StudentPictures/nopic.png' + "?time=" + new Date());
        }
        if (file_exists("../../../documents/StudentSignature/" + data["emp_id"] + ".png")) {
            $('#emp_signature img').attr("src", '../../../documents/StudentSignature/' + data["emp_id"] + ".png" + "?time=" + new Date());
        } else {
            //Picture not exist.
            $('#emp_signature img').attr("src", '../../../documents/StudentSignature/nopic.png' + "?time=" + new Date());
        }
        ...
        fullname=$.trim(data['contact_guardian']).toUpperCase();
        $('div[id="s_mother"] span').text(fullname);
    ...
    function saveStudentInformationList(event) {
        if (event.ctrlKey) {
            return;
        }
//        debugger;
        //validate required fields first
        if ($.trim($('#txtlname').val()) == "" || $.trim($('#txtfname').val()) == "" || $.trim($('#txtmname').val()) == ""
...
//////#3
        ...
        msgBox(message, "Confirmation", "ask", "Yes|No", 300, 250, function (dlgvalue) {
            ...
            if (dlgvalue == 1) {
                animation(1);
                $.ajax({
                    url: "../../../models/mod.cjc.studentrecord.php?ACTION=saveStudentInformation",
                    type: "POST",
                    data: {
                        POSTPARAM: {
                            idnum: $('#txtidnum').val(),
                    ...
                    success: function (recid) {
                        ...
                        if (recid !== "") {
                            ...
                            data.append('POSTPARAM[uploadID]', recid);
                            $.ajax({
                                url: '../../../models/mod.cjc.studentrecord.php?ACTION=uploadImage',
                                data: data,
                                cache: false,
                                contentType: false,
                                processData: false,
                                ...
                                success: function (data) {
                                    ...
                                    msgBox(message, "Success", "success", "OK", 300, 250, function (dlgvalue) {
                                        $('#txtidnum').val(pad(recid, 1));
                                        //LoadStudentInformationListDetails('');
//                                        $('button[name="btnCancel"]').click();
//                                        reloadtblStudentInformationList();
//                                        loadImagesPictures('tblImageSettings');
//                                        loadImagesSignature('tblImageSettingsSign');
                                        GetDetailList(recid, 'idnum');
//                                        if (file_exists("../../../documents/StudentPictures/" + $("#txtemp_id").val() + ".png")) {
//                                            $("#studentprofile_dialog #imgempPic").attr("src", '../../../documents/StudentPictures/' + $("#txtemp_id").val() + ".png" + "?time=" + new Date());
//                                        } else {
//                                            $("#studentprofile_dialog #imgempPic").attr("src", '../../../documents/StudentPictures/nopic.png' + "?time=" + new Date());
//                                        }
//                                        if (dlgvalue == 1) {
        ...
        $("#" + Grid).jqGrid({
            ...
            colModel: [
                ...
                },
                {
                    ...
                    formatter: function (cellvalue, options, rowObject) {
                        var imgSrc = '';
                        if (file_exists("../../../documents/StudentPictures/" + rowObject["Filename"] + "")) {
                            imgSrc = '../../../documents/StudentPictures/' + rowObject["Filename"] + '?' + new Date();
                        } else {
                            imgSrc = '../../../documents/StudentPictures/nopic.png?' + new Date();
                        }
                        return '<img id = "imgemployeeImageGrid-' + cellvalue + '" ' +
                                'src = "' + imgSrc + '"' +
        ...
        $("#" + Grid).jqGrid({
            ...
            colModel: [
                ...
                },
                {
                    ...
                    formatter: function (cellvalue, options, rowObject) {
                        var imgSrc = '';
                        if (file_exists("../../../documents/StudentSignature/" + rowObject["Filename"])) {
                            imgSrc = '../../../documents/StudentSignature/' + rowObject["Filename"] + '?' + new Date();
...
//#2 End Family Background function
    ...
    function checkIsActivePicture(gridID, rowid, chkID) {
        ...
        msgBox("Do you want to set this Picture as Active?", "Confirm", 'ask', 'Yes|No', 0, 0, function (v) {
            if (v == 1) {
                ...
                document.getElementById('img_foldername').value = 'StudentPictures';
    ...
    function checkIsActiveSignature(gridID, rowid, chkID) {
        ...
        msgBox("Do you want to set this Signature to Active?", "Confirm", 'ask', 'Yes|No', 0, 0, function (v) {
            if (v == 1) {
                ...
                document.getElementById('img_filename').value = $('#txtemp_id').val() + '.png';
                document.getElementById('img_filenamenew').value = gridRow["Filename"];
                document.getElementById('img_foldername').value = 'StudentSignature';
...
//:::::::::::::::
    ...
    function setImage(strID) {
        ...
        imgMain.src = '../../../models/mod.cjc.studentrecord.php?ACTION=getImageOriginal&GETPARAM=' + strID;
        ...
        if (input.files && input.files[0]) {
            ...
            if (type == 1) {
                ...
                reader.onload = function (e) {
                    var imgWidth;
                    var imgHeight;
                    var imgStudentThumb = "divPicImageThumb";
                    $("#" + imgStudentThumb).contents().remove();
                    removeAttachement('');
                    var img = document.createElement('img');
                    img.id = 'imgempPic';
                    $(img).css('display:block');
                    img.src = e.target.result;
                    $("#" + imgStudentThumb).append(img);
                    $('img[id="imgempPic"]').css('height', '180px').css('width', '180px').addClass('img-polaroid');
...
//                    $('div[class^="popover"]').eq(0).css({'display': 'none'});
    ...
    function removeAttachement(event) {
        ...
        if (file_exists("../../../documents/StudentPictures/" + $("#txtemp_id").val() + ".jpg")) {
            msgBox("Are you sure to remove attachment?", "Confirm", 'ask', 'YES|NO', 0, 0, function (v) {
                if (v == 1) {
                    $.ajax({
                        url: '../../../models/mod.cjc.studentrecord.php?ACTION=removeAttachment',
                        data: {
...
//:::::::::::::::::::::::
    function LoadDivPrintIDContainer(data) {
           var logoback="blanklogo";
                    if(data['designation']=="Junior High School"){
                        logoback="jrhighschool";
                    }else  if(data['designation']=="Senior High School"){
                        logoback="srhighschool";
                    }
        var strDivID = '\
      <div style="text-align:center;display:inline-block;" id="div_print_id">\n\
         <div style="width:auto;">\n\
               <div id="_front" style="display:inline-block;border-right:black solid thin;   padding: 20px 27px 30px 20px;">\n\
                  <div id="div_img_id_front" style="border-radius:10px;border: rgba(153, 153, 153, 0) dashed 0.5px;width:' + data['idwidth'] + ';height:' + data['idheight'] + ';position:relative;float:left;">\n\
                      ...
                      <div id="emp_pic" style="width:' + data['picwidth'] + ';height:' + data['picheight'] + ';top:0px;position:absolute;margin-top: 81px;right: 8px;text-align: center;">\n\
                          <img src="../../../documents/StudentPictures/1.png" style="width:' + data['picwidth'] + ';height:' + data['picheight'] + ';border: #C15858 solid thin;border-color: transparent;border-radius: 5px;" alt="">\n\
                      </div> \n\
                      ...
                      <div id="emp_name" style="top:0px;position:absolute;margin-top: 185px;font-weight: bold;margin-left: 0px;text-align: right;right: 1px;     width: 100%; -webkit-transform: scale(0.89051,1.013570);"> \n\
                     ...
                     <div id="emp_position" style="top:0px;position:absolute;margin-top: 199px;font-weight: bold;margin-left: 0px;text-align: right;right: 1px; width: 85%; -webkit-transform: scale(0.89051,1.013570);"> \n\
                         <span style="font-size:10px;" ></span>\n\
                     </div> \n\
              ...
              <div id="_back" style="display: inline-block;    padding: 20px 0px 30px 20px;"> \n\
                 <div id="div_img_id_back" style="border-radius:10px;border: rgba(153, 153, 153, 0) dashed 0.5px;width:' + data['idwidth'] + ';height:' + data['idheight'] + ';position:relative;float:right;"> \n\
                    ...
                    <div id="logoback" style="top:0px;position:absolute;margin-top: 74px;font-weight: bold;margin-left: 0px;text-align: right;right: 6px; width: 100%; ">\n\
                        <img src="../../../documents/zzzIDSetupImage/' + logoback + '.png" style="width:50px;height:50px;border: #C15858 solid thin;border-color: transparent;border-radius: 5px;" alt="">\n\
                     </div>\n\
                     <div id="s_SSS" style="display:none;position: absolute;top: 0;margin-top: 25px;left: 55px;font-size: 12px;width: 80%;text-align: left;-webkit-transform: scale(0.7081,0.9175);font-weight: bold;"> <span></span></div>\n\
                     <div id="s_tinno" style="display:none;position: absolute;top: 0;margin-top: 45px;left: 55px;font-size: 12px;width: 80%;text-align: left;-webkit-transform: scale(0.7081,0.9175); font-weight: bold;"> <span></span></div>\n\
                     <div id="s_philhealth" style="display:none;position: absolute;top: 0;margin-top: 65px;left: 55px;font-size: 12px;width: 80%;text-align: left;-webkit-transform: scale(0.7081,0.9175); font-weight: bold;"> <span></span></div> \n\
                    <div id="s_birthdate" style="position: absolute;top: 0;margin-top: 85px;left: 55px;font-size: 12px;width: 80%;text-align: left;-webkit-transform: scale(0.7081,0.9175); font-weight: bold;">  <span></span></div>\n\
                     <div id="p_civil" style="display:none;position: absolute;top: 0;margin-top: 106px;left: 55px;font-size: 12px;width: 80%;text-align: left;-webkit-transform: scale(0.7081,0.9175); font-weight: bold;"> <span></span></div>\n\
                     ...
                     <div id="p_cellno" style="position: absolute; top: 0;margin-top: 202px; left: 55px; font-size: 12px; width: 80%; text-align: left; transform: scale(0.7081, 0.9175); font-weight: bold;"> <span> </span></div>\n\
                     <div id="emp_signature" style="width: 100%;position: relative;top: 0;margin-top: -60px;-webkit-transform: scale(0.91,0.975); font-weight: bold;">\n\
                         <img src="../../../documents/StudentSignature/1.png?Thu Sep 03 2015 21:51:26 GMT-0700 (Pacific Daylight Time)" style="/*width: 160px;height: 35px;*/height:50px;border: #C15858 solid thin;border-color: transparent;border-radius: 5px;" alt=""> \n\
                     </div> \n\
                </div>\n\
             </div> \n\
          </div> \n\
...
//        $('div[class^="popover"]').eq(0).css({'display': 'none'});
        $('button[id="idpopover"]').click();
        var dwidth = $(window).width() - 180;
        var dheight = $(window).height() - 20;
                ...
                .dialog({
                    ...
                    open: function () {
                        ...
                        $(this).load(function () {
                            ...
                            frame.foldername = 'StudentPictures';

                            frame.loadInitialCanvas($('#txtemp_id').val(), 'StudentPictures');
//                            frame.$('button[name="btnPrintID"]').click();
//                            frame.$('button[name="btnPrintID"]').click();
                            if ($('img[id="imgempPic"]').attr('src').split('nopic.png').length == 1) {
                                msgBox("Do you want to replace Picture?", "Confirm", 'ask', 'Yes|No', 0, 0, function (v) {
                ...
                .dialog({
                    ...
                    open: function () {
                        ...
                        $(this).load(function () {
                            ...
                            console.log(frame.filename, $(this));
                            frame.filename = $('#txtemp_id').val();
                            frame.foldername = 'StudentSignature';

                            frame.loadInitialCanvas($('#txtemp_id').val(), 'StudentSignature');
...
//                            frame.$('button[name="btnPrintID"]').click();
    ...
    function toUpperFirstWord(str) {
        ...
        for (i = 0; i < sp.length; i++) {
            if (sp[i] == "del" || sp[i].substring(0, 1) == "I" || sp[i].substring(0, 2) == "II" || sp[i].substring(0, 3) == "III" || sp[i].substring(0, 2) == "IV" || sp[i].substring(0, 1) == "V" || sp[i].substring(0, 2) == "VI" || sp[i].substring(0, 3) == "VII" || sp[i].substring(0, 3) == "VIII" || sp[i].substring(0, 2) == "IX" || sp[i].substring(0, 1) == "X") {
...
<div  class="container-fluid">
    <div id="divIDFilterStudentContainer" class="panel panel-default" style="margin: 0px;">
                        ...
                        <div class="row-fluid">                          
                            <div class="col-xs-8 text-left" style=" padding: 0px;width:75%;">                                
                                ...
                                <div id="divFilterPrint" class="col-xs-1" style="padding: 0px;width: 15%;">
                                    ...
                                    <div id="studentfilterprintbtnFilters" class="btn-group" style="padding: 0px;padding-left: 15%;">
                                        ...
                                        <ul id="studentfilterprintulIDsDropdowns" class="dropdown-menu" role="menu" style="min-width: 120px;margin-left: 13%;">
                                ...
                                <div class="col-xs-4" style=" padding:0px 0px 0px 2.7%;width: 30%;">
                                    <div class="input-group">
                                        <input id="txtSearchStudent" class="form-control txt-standard" type="text" placeholder="Search" style="height: 28px;">
                                        <span class="input-group-addon" style="padding: 0px;">
                                            <button name="btnSearchStudent" id="<?= $strModuleName ?>-LBL_REFRESH" title="click to refresh" class="btn btn-default btn-md btn-standard" style="padding: 1px 7px;height: 25px;">
                            ...
                            <div class="col-xs-4" style="float:right;margin-top: -2px;padding-right: 0;width:25%;">
                                ...
                                <button  id="<?= $strModuleName ?>-LBL_NEW" name="btnNew"  disabled="disabled" class="btn btn-info btn-lg" title="New" style="display:none;padding: 7px 12px;float: right;font-size: 12px;margin-left:1.5%;width:26%;">
    ...
    <div class="panel-body" id="panel-body" style="padding:0px;">
        <div id="divStudentInformationList" oncontextmenu="return false;">
            <table id="tblStudentInformationList"></table>
            <div id="pgStudentInformationList"></div>
        </div>
    </div>
</div>
<div class="panel panel-default" id="studentprofile_dialog" style="display: none;padding: 0px;">
    ...
    <div class="panel-heading" id="divTopHeader" style="height: 40px;width: 100%;padding: 0px;">
        <div class="col-md-6" style="float:left;margin-top:3px;"> 
            ...
            <button  id="<?= $strModuleName ?>-LBL_SAVE"  name="btnSaveStudent" class="btn btn-info btn-lg" title="Save" style="padding: 7px 12px;float: left;font-size: 12px;margin-right: 0.5%;       width: 15%;">
                    ...
                    <div class="col-xs-10" style="padding: 0;">
                        <div class="row-fluid">
                            <div class="col-xs-3" style="padding-right: 0px;">
                                <div class="input-group">
                                    <span class="input-group-addon" style="text-align: left;">
                                        <label  id="<?= $strModuleName; ?>-LBL_FRMSTUDENTID" style="    min-width: 84px;"><?= $LBL_FRMSTUDENTID; ?></label>
                                    </span>
                                    <input  id="txtidnum" type="hidden"/>
                                    <input name="ID" class="form-control"  type="text" id="txtemp_id" readonly style="text-align: center;"/>
                                </div>
                            </div> 
                            <div class="row-fluid col-xs-9" style="padding: 0px; ">
                                <div class="col-xs-12"> 
                                    <div class="input-group col-xs-2" style="border-bottom-left-radius: 4px;border-top-left-radius: 4px;border-left: 1px solid #ccc;height: 28px;border-bottom-right-radius: 0px;  border-top-right-radius: 0px;  padding: 0px 0px 0px 0px;">
                                        <span class="input-group-addon " style="padding:0px;padding-left: 10px;"><label id="<?= $strModuleName ?>-LBL_FRMSTUDENTNAME" style="min-width: 123px;float: left;text-align: left;"><?= $LBL_FRMSTUDENTNAME ?></label></span>
                                    </div>                       
        ...
        <div class="container-fluid text-left" id="divTabs">
            ...
            <div class="tab-content" id='divTabContent' style="width:100%;height:290px;overflow: overlay;padding-right: 17px;" >
                ...
                <div class="panel panel-default tab-pane fade " id="tab-IDPREVIEW" style="border: none;">                 
                    ...
                    <button id="btnRefreshSignature" style="display:none;" onclick="functionRefreshStudentSignature();">Refresh Signature</button>
                                ...
                                <div id="_front" style="display:inline-block;border-right:black solid thin;   padding: 10px 20px 30px 20px;">
                                    <div id="div_img_id_front" style="border-radius:10px;border: rgba(153, 153, 153, 0) dashed 0.5px;width: 5.4cm;height: 8.54cm;position:relative;float:left;">
                                        ...
                                        <div id="emp_pic" style="width: 100px;height: 100px;top:0px;position:absolute;margin-top: 81px;right: 8px;text-align: center;">
                                            <img src="../../../documents/StudentPictures/nopic.png?time=Fri Mar 04 2016 00:00:42 GMT-0800 (Pacific Standard Time)" style="width: 97px;height: 97px;border: #C15858 solid thin;border-color: transparent;border-radius: 5px;" alt="">
                                        </div> 
                                ...
                                <div id="_back" style="display: inline-block;    padding: 10px 0px 30px 20px;"> 
                                    <div id="div_img_id_back" style="border-radius:10px;border: rgba(153, 153, 153, 0) dashed 0.5px;width: 5.4cm;height: 8.54cm;position:relative;float:right;"> 
                                        ...
                                        <div id="emp_signature" style="width: 100%;position: relative;top: -60px;-webkit-transform: scale(0.91,0.975); font-weight: bold;">
                                            <img src="../../../documents/StudentSignature/0017-0222-5.png?time=Fri Mar 04 2016 00:00:42 GMT-0800 (Pacific Standard Time)" style="width: 160px;height: 35px;border: #C15858 solid thin;border-color: transparent;border-radius: 5px;" alt=""> 
                                        </div> 
                                    </div>
                                </div> 
                            </div>
                                                            ...
                                                            <div id="_front" style="display:inline-block;border-right:black solid thin;   padding: 10px 20px 30px 20px;">
                                                                <div id="div_img_id_front" style="border-radius:10px;border: rgba(153, 153, 153, 0) dashed 0.5px;width: 5.43cm;height: 8.58cm;position:relative;float:left;">
                                                                    ...
                                                                    <div id="emp_pic" style="width: 100px;height: 100px;top:0px;position:absolute;margin-top: 81px;right: 10px;text-align: center;">
                                                                        <img src="../../../documents/StudentPictures/nopic.png?time=Wed Sep 16 2015 20:33:52 GMT-0700 (Pacific Daylight Time)" style="width: 100px;height: 100px;border: #C15858 solid thin;border-color: transparent;border-radius: 5px;" alt="">
                                                                    </div> 
                                                            ...
                                                            <div id="_back" style="display: inline-block;  padding: 10px 0px 30px 20px;"> 
                                                                <div id="div_img_id_back" style="border-radius:10px;border: rgba(153, 153, 153, 0) dashed 0.5px;width: 5.43cm;height: 8.58cm;position:relative;float:right;"> 
                                                                    ...
                                                                    <div id="emp_signature" style="width: 100%;position: relative;top: -47px;-webkit-transform: scale(0.91,0.975); font-weight: bold;">
                                                                        <img src="../../../documents/StudentSignature/2009-0418-8.png?Thu Sep 03 2015 21:51:26 GMT-0700 (Pacific Daylight Time)" style="width: 128px;height: 20px;border: #C15858 solid thin;border-color: transparent;border-radius: 5px;" alt=""> 
                                                                    </div> 
                                                                </div>
                                                            </div> 
                                                        </div> -->
...
<!--                        <div id="theBox"><span>This is the content</span></div>
    ...
    <form method="post" accept-charset="utf-8" name="form1StudentSignature" style="display:none;">    
...
<div class="contextMenu" class="vmenu"  id="StudentRecordContextRmenu"  style="display:none;">
