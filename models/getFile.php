...
if(!empty($_POST["file_location"])){
    $pl = $_POST["file_location"] == "10.8.3.228"?"ftp://casfiles:casfiles@":"ftp://contractor:contractor@";
    $filePath = $pl.$_POST["file_location"]."/".$_POST["download_file"]; // set your download file path here.
    $tf = iconv("UTF-8","SJIS-win",$filePath);
    $ORIG = $_POST["download_file"];
    download($tf); // calls download function
}else{
    ...
    $fileEx   = $pth['extension'];
    $default_cont = "application/octet-stream";
    ...
    if (array_key_exists($fileEx, $content_types))
    {
        $content = $content_types[$fileEx];
    }
    else
    {
        $content =  $default_cont;
    }
    $ORIG = $filename;
...
function download($tf)
{
    if(!empty($tf))
    {
        $fileInfo = pathinfo($tf);
        ...
        if(file_exists($tf))
        {
            ...
            header("Content-Disposition: attachment;filename=".$_POST["download_file"]);
            header('Content-Type: '.$contentType.'; charset=Shift_JIS');
            header("Accept-Ranges: bytes");
            header("Pragma: public");
            header("Expires: -1");
            header("Cache-Control: no-cache");
            header("Cache-Control: public, must-revalidate, post-check=0, pre-check=0");
            header("Content-Length: ".filesize($tf));
            $chunksize = 8 * (1024 * 1024); //8MB (highest possible fread length)
            if ($size > $chunksize)
            {
                $handle = fopen(mkdir("C:\\pcatemp\\".$_POST["download_file"]), 'rb');
                $buffer = '';
