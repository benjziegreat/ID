...
class ModCJCStudentRecord extends Model {

    private $conn = null;
//    private $path = "C:/AppServ/www/CJC_IDSYSTEM/documents/StudentPictures/";
    private $path1 = "../documents/StudentPictures/";
    private $path2 = "../documents/StudentSignature/";
    ...
    public function __construct() {
        ...
        switch ($this->action) {
            ...
            case 'saveStudentInformation':
                echo $this->saveStudentInformation($this->postparams);
                break;
...
if ($strSQLinject != "") {
    ...
    public function loadList($param) {
        $strSQLa = "INSERT INTO cjc_idsystem.student
                    SELECT
                    null `idnum`,
                    stud.`STDNTID`  `emp_id`,
                    stud.`LASTNAME` `lname`,
                   ...
                   'System Generated'  `addedby`,
                    ...
                    FROM
                      ...
                      where  stud.`STDNTID` not IN(select emp_id from cjc_idsystem.student)  # limit 0,1000
...
//        IsAllList IsForPrinting chkIsPrintedList IsNoPicture  IsWithPicture IsNoSignature IsWithSignature
        ...
        $strSQL = "SELECT 
                  ...
                  ifnull(rec.`lname`,'') lname, 
                  (CASE rec.`sex`  WHEN 0 THEN 'Female' WHEN 1 THEN 'Male'  END) as Sex,
                  ifnull(rec.`fname`,'') fname,
                  ifnull(rec.`mname`,'') mname,
                  ifnull(rec.`nameext`,'') nameext,
    ...
    public function GetDetailList($param) {
        ...
        if ($param["type"] == "idnum") {
            ...
            $wherefilterby = "where rec.`idnum` in('{$param["recid"]}') ";// 05132025: change emp_id to idnum mar 13 2025
        }
        $strSQL = "                 
               SELECT tbl.*
               FROM(
                    ...
                    FROM 
                      `{$dbname}`.`student` rec
                        ...
                        *,'Student' typeid
                        ...
                        where setuptype='Student' and setdefault=1
                    # )zid ON((rec.typeid=zid.typeID and rec.category=zid.category) OR  IF(rec.typeid=zid.typeID and (rec.category='' or rec.category is null),zid.category='' or zid.category is null,''))
                    ...
                    {$wherefilterby}
                      ...
                      zid.`backsignaturexy`
                        ...
                        where zid.setuptype='Student' and zid.setdefault=1  and zid.category='COLLEGE' 
               )tbl                   
                    ";
    ...
    public function GetImagePictures($param) {
      $dbname = DB_NAME;
        $strSQL = "SELECT
            ...
            FROM
              `$dbname`.`zpicturerecord` where emp_id='{$param["emp_id"]}' and isdeleted=0 and idtype='Student'
               ";
        return $this->getJSonJQGridPagingResponse($strSQL, null, false);
        // return $this->getJSonJQGridResponseLimit($strSQL, null, false);
    }
    ...
    public function GetImageSignature($param) {
      $dbname = DB_NAME;
        $strSQL = "SELECT
              ...
              if(concat(emp_id,'.png')=filename,1,0) isactive
            FROM
              `$dbname`.`zsignaturerecord` where emp_id='{$param["emp_id"]}' and isdeleted=0 and idtype='Student'
               ";
        return $this->getJSonJQGridPagingResponse($strSQL, null, false);
        // return $this->getJSonJQGridResponseLimit($strSQL, null, false);
    }
    
    public function saveStudentInformation($param) {
        $dbname = DB_NAME;
        $this->db->autocommit(FALSE);
        $strNewID = ($param["idnum"] == "" ? 'NULL' : $param["idnum"]);
        if ($strNewID == 'NULL') {
            $strSQL = "INSERT INTO `$dbname`.student SET                           
                          idnum={$strNewID},
            ...
            $strSQL = "UPDATE `$dbname`.student SET
                           emp_id='" . $this->db->real_escape_string($param["emp_id"]) . "',
                          lname='" . $this->db->real_escape_string($param["lname"]) . "',
                          fname='" . $this->db->real_escape_string($param["fname"]) . "',
                          mname='" . $this->db->real_escape_string($param["mname"]) . "',
...
//        mb_convert_variables("UTF-8", "SJIS-win", $strSQL);
        ...
        if ($strResult) {
            //Initialized values ::::::::::::::::::::::::::::::::::::::::::
            $strSQL = "select last_insert_id(idnum) as id from `$dbname`.student order by id desc limit 1";
...
//        mb_convert_variables("UTF-8", "SJIS-win", $strSQL);
        ...
        if ($strResult) {
            ...
            if ($param["type"] == 'Individual') {
                $strSQL = "select printinghistory  from `$dbname`.student  WHERE idnum  in({$strNewID})";
                $arrLastInsertedID = $this->db->fetchAll($strSQL);
    ...
    public function uploadImage($strID) {
        $dbname = DB_NAME;
        $strSQL = "select (emp_id) as id from `$dbname`.student  where idnum='{$strID}'";
